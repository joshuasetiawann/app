    ...
    <aside style={pcss('display:flex;flex-direction:column;gap:14px;padding:18px 18px 26px;border-left:1px solid var(--ln,rgba(74,74,74,.08));background:var(--bg,#FDFBF7);overflow-y:auto')}>
      ...
      <div style={pcss('border-radius:22px;background:var(--sf,#fff);padding:15px 16px;box-shadow:var(--shadow,0 8px 24px rgba(0,0,0,.04))')}>
        <div style={{ display: 'flex', justifyContent: 'space-between', gap: 8, alignItems: 'center', marginBottom: 11 }}>
          <div style={pcss("font:700 12.5px 'Quicksand',sans-serif;color:var(--ink,#4A4A4A)")}>Schedule · {today}</div>
          <button type="button" onClick={() => navigate('/schedule')} style={pcss("border:0;background:transparent;padding:2px;color:var(--pki,#E86F87);font:800 10px 'Nunito',sans-serif;cursor:pointer")}>View all</button>
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 11 }}>
          ...
          {upcoming.length === 0 && (
            <button type="button" onClick={() => navigate('/schedule')} style={pcss("width:100%;padding:12px;border:1px dashed var(--ln,rgba(74,74,74,.12));border-radius:14px;background:transparent;color:var(--mut,#A99A9E);font:600 10.5px/1.45 'Nunito',sans-serif;cursor:pointer")}>No events yet. Add your first shared plan.</button>
          )}
        </div>
      </div>
      ...
      <button
        type="button"
        aria-label={partner ? `Open ${partnerName}'s location` : 'Connect partner'}
      ...
      >
        ...
        <div style={{ padding: '13px 16px' }}>
          <div style={pcss("font:700 12.5px 'Quicksand',sans-serif;color:var(--ink,#4A4A4A)")}>{partner ? `${partnerName}'s location` : "Partner's location"}</div>
          <div style={pcss("font:600 10.5px 'Nunito',sans-serif;color:var(--mut,#A99A9E);margin-top:3px")}>{partner ? 'Open to view the latest location or start sharing' : 'Connect your partner to share location with permission'}</div>
        </div>
      </button>
    </aside>
  );
