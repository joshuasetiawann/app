...
export default function HomePage() {
  ...
  const { theme, foodStatus, partnerFoodStatus, setFoodStatus, toast, mood, activity, partnerMood, partnerActivity, events, photos, countdowns, loveNotes } = useAppState();
  ...
  const featuredNote = loveNotes.find((note) => note.from === 'partner' && note.state !== 'done')
    ?? loveNotes.find((note) => note.state !== 'done')
    ?? loveNotes[0];
    ...
    <ScrollColumn>
      ...
      <HeroSurface background={heroBg} style={pcss('border-radius:26px;padding:20px 18px 18px;position:relative;overflow:hidden;box-shadow:0 8px 26px rgba(255,140,150,.16)')}>
        ...
        <div style={{ marginTop: 16, paddingTop: 14, borderTop: '1px dashed rgba(255,255,255,.85)', textAlign: 'center', position: 'relative' }}>
          ...
          <div style={pcss("font:500 18px 'Caveat',cursive;color:var(--ink2,#6B5B60);margin-top:5px")}>
            Since {relationshipDate(couple?.startedAt || RELATIONSHIP.startedAt)} 💗
          </div>
        </div>
      </HeroSurface>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: twoCol, gap: 12 }}>
        ...
        <div style={pcss('border-radius:24px;padding:16px 17px;background:linear-gradient(155deg,#4A3B45,#2E2530);color:#FFF6F3;box-shadow:0 8px 22px rgba(60,45,52,.25)')}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <span style={pcss("font:700 11px 'Nunito',sans-serif;color:rgba(255,246,243,.6)")}>{nextCountdown ? nextCountdown.title.toUpperCase() : 'NEXT MOMENT'} {nextCountdown ? nextCountdown.icon : '✨'}</span>
            <span style={pcss("font:700 10px 'Nunito',sans-serif;color:#FFB7B2")}>{nextCountdown ? new Intl.DateTimeFormat('en-US', { day: 'numeric', month: 'short' }).format(new Date(nextCountdown.targetDate)) : 'Not set'}</span>
          </div>
          <div style={{ display: 'flex', alignItems: 'flex-end', gap: 8, marginTop: 10 }}>
            <span style={pcss("font:700 38px/.9 'Quicksand',sans-serif;color:#FFB7B2")}>{nextCountdown ? daysToNext : '—'}</span>
            <span style={pcss("font:500 17px 'Caveat',cursive;color:rgba(255,246,243,.8);padding-bottom:4px")}>{nextCountdown ? `${daysToNext === 1 ? 'day' : 'days'} left` : 'create your first countdown'}</span>
          </div>
          <div style={{ height: 6, borderRadius: 6, background: 'rgba(255,255,255,.16)', marginTop: 14 }}>
            <div style={{ width: `${nextCountdown ? Math.min(100, Math.max(4, nextCountdown.progressPercent)) : 0}%`, height: 6, borderRadius: 6, background: 'linear-gradient(90deg,#FFB7B2,#E3D7F7)', transition: 'width .35s ease' }} />
          </div>
          <div style={pcss("font:600 10px 'Nunito',sans-serif;color:rgba(255,246,243,.6);margin-top:9px")}>{nextCountdown ? nextCountdown.when : 'Add an important moment to see it here.'}</div>
          <button
            type="button"
      ...
      <div style={{ display: 'grid', gridTemplateColumns: twoCol, gap: 12 }}>
        <Card>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <button type="button" style={pcss("border:0;background:transparent;padding:4px;font:700 10.5px 'Nunito',sans-serif;color:var(--pki,#E86F87);cursor:pointer")} onClick={() => navigate('/schedule')}>Calendar</button>
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 11, marginTop: 13 }}>
            {events.slice(0, 3).map((event) => (
              <UpcomingRow
                ...
                right={event.scope === 'Berdua' ? 'Shared' : 'Personal'}
        ...
        <button type="button" style={pcss('border-radius:24px;background:var(--sf2,#FFF4F1);border:1px dashed rgba(232,111,135,.45);padding:16px 17px;cursor:pointer;text-align:left;color:inherit')} onClick={() => navigate('/notes')}>
          ...
          <div style={pcss("margin-top:12px;display:inline-block;padding:8px 15px;border-radius:100px;background:var(--sf,#fff);font:700 11.5px 'Nunito',sans-serif;color:var(--pki,#E86F87)")}>
            {featuredNote ? 'View love note' : 'Write a love note'}
          </div>
        </button>
      </div>
    </ScrollColumn>
