...
export default function FoodPage() {
  ...
  const foodCategories = [...new Set([
    ...FOOD_CATS.filter((category) => category !== 'All'),
    ...foodAlbums.map((album) => album.title.slice(FOOD_ALBUM_PREFIX.length)),
    ...foodEntries.map((entry) => entry.category),
  ])];
  const foodFilters = ['All', ...foodCategories];
  const visibleEntries = foodCategory === 'All' || foodCategory === 'Semua' ? foodEntries : foodEntries.filter((f) => f.category === foodCategory);
  ...
  const deleteFoodAlbum = (id: string) => {
    ...
    foodEntries.filter((entry) => entry.category === title).forEach((entry) => updateFoodEntry(entry.id, { category: 'Other' }));
    removeAlbum(id);
    if (foodCategory === title) setFoodCategory('All');
    setConfirmDeleteAlbumId('');
    toast('Album deleted; its entries were moved to Other');
  };
    ...
    <ScrollColumn>
      <HeroSurface background={heroBg} style={pcss('border-radius:24px;padding:18px;box-shadow:0 8px 24px rgba(255,140,150,.14)')}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          <div>
            <div style={pcss("font:700 20px 'Quicksand',sans-serif;color:var(--ink,#4A4A4A)")}>Have you eaten yet?</div>
            <div style={pcss("font:500 18px 'Caveat',cursive;color:var(--ink2,#6B5B60);margin-top:2px")}>take care of yourself for me 🥺</div>
          </div>
        ...
        <div style={{ display: 'flex', gap: 8, marginTop: 15, flexWrap: 'wrap' }}>
          <button type="button" style={foodBtnStyle(foodStatus === 'ate')} onClick={() => { setFoodStatus('ate'); toast('Meal status saved · feeling full 😋'); }} aria-pressed={foodStatus === 'ate'}>I already ate 😋</button>
      ...
      <SectionHeader title="Food journal" action="+ Add a meal" onAction={() => openSheet('food')} />
      <div style={{ display: 'flex', flexDirection: 'column', gap: 11 }}>
        {visibleEntries.length === 0 && (
          <div style={pcss('border-radius:22px;background:var(--sf,#fff);padding:20px;text-align:center;box-shadow:var(--shadow,0 8px 24px rgba(0,0,0,.04))')}>
            ...
            <div style={pcss("font:700 12.5px 'Nunito',sans-serif;color:var(--ink,#4A4A4A);margin-top:8px")}>No meals in this category yet</div>
          </div>
        )}
            ...
            <div key={f.id} style={pcss('border-radius:22px;background:var(--sf,#fff);padding:13px;box-shadow:var(--shadow,0 8px 24px rgba(0,0,0,.04))')}>
              ...
              >
                <div style={pcss('width:76px;height:76px;border-radius:16px;background:repeating-linear-gradient(135deg,#EFE6E2 0 8px,#F8F2EE 8px 16px);display:flex;align-items:center;justify-content:center;font-size:22px;flex:none')}>
                  {f.imageUrl ? <img src={f.imageUrl} alt={`${f.name}`} style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : f.icon}
                </div>
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={pcss("font:600 10.5px 'Nunito',sans-serif;color:var(--mut,#A99A9E);margin-top:3px")}>
                    {f.by === 'me' ? (profile?.nickname || profile?.name || 'You') : (partner?.nickname || partner?.name || 'Partner')} · {f.location} · {f.category} · {f.price}
                  </div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingTop: 4 }}>
        <span style={pcss("font:700 15px 'Quicksand',sans-serif;color:var(--ink,#4A4A4A)")}>Meal collections 🍱</span>
        <button type="button" onClick={() => setManagingAlbums((value) => !value)} aria-expanded={managingAlbums} style={pcss("border:0;background:transparent;padding:4px;font:800 11px 'Nunito',sans-serif;color:var(--pki,#E86F87);cursor:pointer")}>{managingAlbums ? 'Done' : 'Manage'}</button>
      </div>
      ...
      {managingAlbums && (
        <section style={pcss('padding:14px;border-radius:20px;background:var(--sf2,#FFF4F1);border:1px solid var(--ln,rgba(74,74,74,.1));animation:kk-soft-in .24s ease')}>
          <form onSubmit={createFoodAlbum} style={{ display: 'grid', gridTemplateColumns: '1fr auto', gap: 8 }}>
            <input value={newAlbumTitle} onChange={(event) => setNewAlbumTitle(event.target.value)} aria-label="New meal collection name" placeholder="e.g. Bandung favorites" maxLength={40} required style={compactInput} />
            <button type="submit" style={compactButton}>+ Add</button>
          </form>
          <small style={pcss("display:block;margin-top:7;font:600 9.5px/1.45 'Nunito',sans-serif;color:var(--mut,#A99A9E)")}>Built-in categories stay available. Custom collections can be renamed or deleted and sync with your partner.</small>
            ...
            {foodAlbums.map((album) => {
              ...
              if (editingAlbumId === album.id) return (
                <form key={album.id} onSubmit={saveFoodAlbum} style={{ display: 'grid', gridTemplateColumns: '1fr auto', gap: 8 }}>
                  <input value={editAlbumTitle} onChange={(event) => setEditAlbumTitle(event.target.value)} aria-label={`Rename ${title}`} maxLength={40} required style={compactInput} />
                  <button type="submit" style={compactButton}>Save</button>
                </form>
              );
              return (
                <div key={album.id} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  ...
                  <button type="button" style={compactButton} onClick={() => { setEditingAlbumId(album.id); setEditAlbumTitle(title); setConfirmDeleteAlbumId(''); }}>Edit</button>
                  <button type="button" style={{ ...compactButton, color: '#B5485D' }} onClick={() => confirmDeleteAlbumId === album.id ? deleteFoodAlbum(album.id) : setConfirmDeleteAlbumId(album.id)}>{confirmDeleteAlbumId === album.id ? 'Confirm' : 'Delete'}</button>
                </div>
              );
            })}
            {!foodAlbums.length && <small style={pcss("font:600 10px 'Nunito',sans-serif;color:var(--mut,#A99A9E)")}>No custom collections yet.</small>}
          </div>
        </section>
      )}
        ...
        {foodCategories.map((category, index) => {
          ...
          return (
            <button type="button" key={category} onClick={() => setFoodCategory(category)} style={{ flex: 'none', width: 112, padding: 0, border: 0, background: 'transparent', textAlign: 'left', color: 'inherit', cursor: 'pointer' }}>
              ...
              <div style={pcss("font:600 9.5px 'Nunito',sans-serif;color:var(--mut,#A99A9E)")}>{entries.length} entries{entries.some((entry) => entry.imageUrl) ? ' · with photos' : ''}</div>
            </button>
          );
        })}
      </div>
      ...
      <div style={pcss('border-radius:22px;background:var(--sf,#fff);padding:16px 17px;box-shadow:var(--shadow,0 8px 24px rgba(0,0,0,.04))')}>
        <div style={pcss("font:700 13.5px 'Quicksand',sans-serif;color:var(--ink,#4A4A4A)")}>Favorite meal snapshot</div>
        <div style={pcss("font:600 10px 'Nunito',sans-serif;color:var(--mut,#A99A9E);margin-top:3px")}>Automatic insights from your journal</div>
