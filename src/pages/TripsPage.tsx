  ...
  return (
    <ScrollColumn>
      <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
        <button type="button" onClick={() => setCreating((value) => !value)} aria-expanded={creating} style={pcss("padding:9px 14px;border:0;border-radius:100px;background:var(--sf2,#FFF4F1);color:var(--pki,#E86F87);cursor:pointer;font:800 10.5px 'Nunito',sans-serif")}>+ Plan a trip</button>
      </div>

      {creating && (
        <QuickCreatePanel
          title="Plan a trip"
          description="These basics are shared with your partner; travel documents can stay in Files."
          submitLabel="Save trip ✈️"
          fields={[
            { name: 'title', label: 'Trip name', placeholder: 'Meet in Bali', required: true, wide: true },
            { name: 'startsOn', label: 'Start', type: 'date', min: today },
            { name: 'endsOn', label: 'End', type: 'date', min: today },
            { name: 'flightCode', label: 'Flight code', placeholder: 'GA-412' },
            { name: 'departLabel', label: 'Departing from', placeholder: 'CGK · 08:30' },
            { name: 'arriveLabel', label: 'Arriving at', placeholder: 'DPS · 11:20' },
          ]}
          ...
          onSubmit={(values) => {
            ...
            setCreating(false);
            toast('Trip saved for both of you ✈️');
          }}
        />
      )}

      {trips.length === 0 && !creating && <EmptyState tag="PLANS FOR TWO" emoji="🧳" title="No trips yet" body="Create your first plan; dates and flight details will appear on both accounts." actionLabel="Plan a trip" onAction={() => setCreating(true)} />}
      ...
      {upcoming && (
        <div style={pcss('border-radius:26px;overflow:hidden;background:var(--sf,#fff);box-shadow:var(--shadow,0 8px 24px rgba(0,0,0,.04))')}>
          <div style={{ height: 170, background: upcoming.coverGradient, position: 'relative', display: 'flex', alignItems: 'flex-end', padding: 16 }}>
            <div style={pcss("position:absolute;top:14px;left:16px;background:#fff;border-radius:100px;padding:5px 11px;font:800 9.5px 'Nunito',sans-serif;color:var(--pki,#E86F87)")}>UPCOMING</div>
          ...
          <div style={{ padding: '16px 17px' }}>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(110px,1fr))', gap: 12 }}>
              {[['FLIGHT', upcoming.flightCode], ['DEPARTING', upcoming.departLabel], ['ARRIVING', upcoming.arriveLabel]].map(([label, value]) => <div key={label}><div style={pcss("font:700 9px 'Nunito',sans-serif;color:var(--mut,#A99A9E)")}>{label}</div><div style={pcss("font:700 12px 'Nunito',sans-serif;color:var(--ink,#4A4A4A);margin-top:3px")}>{value}</div></div>)}
              <div><div style={pcss("font:700 9px 'Nunito',sans-serif;color:var(--mut,#A99A9E)")}>FILES</div><button type="button" onClick={() => navigate('/files')} style={pcss("border:0;background:transparent;padding:3px 0;font:700 12px 'Nunito',sans-serif;color:var(--pki,#E86F87);cursor:pointer")}>View files ›</button></div>
            </div>
