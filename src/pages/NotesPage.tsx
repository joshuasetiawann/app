  ...
  return (
    <ScrollColumn>
      <HeroSurface background={heroBg} style={pcss('border-radius:24px;padding:18px')}>
        ...
        <button type="button" onClick={() => setCreating((value) => !value)} aria-expanded={creating} style={pcss("margin-top:14px;padding:10px 16px;border:0;border-radius:100px;background:rgba(255,255,255,.64);font:800 11px 'Nunito',sans-serif;color:var(--pki,#E86F87);cursor:pointer")}>+ Tulis surat baru</button>
      </HeroSurface>

      {creating && (
        <QuickCreatePanel
          title={`Surat untuk ${partner?.nickname || partner?.name || 'pasanganmu'}`}
          description="Kosongkan waktu buka jika surat boleh langsung dibaca."
          submitLabel="Kirim surat 💌"
          fields={[
            { name: 'preview', label: 'Judul / kalimat depan', placeholder: 'Buka saat kamu kangen…', required: true, wide: true },
            { name: 'unlockAt', label: 'Boleh dibuka mulai', type: 'datetime-local', defaultValue: tomorrow, wide: true },
            { name: 'body', label: 'Isi surat', type: 'textarea', placeholder: 'Tulis semua yang ingin kamu sampaikan…', required: true, wide: true },
          ]}
          ...
          onSubmit={(values) => {
            ...
            if (!added) return;
            setCreating(false);
            toast('Surat terkirim dan disimpan aman 💌');
          }}
        />
      )}

      {loveNotes.length === 0 && !creating && <EmptyState tag="SURAT UNTUK KALIAN" emoji="💌" title="Belum ada surat" body="Tulis surat pertama yang bisa dibuka sekarang atau pada waktu pilihanmu." actionLabel="Tulis surat pertama" onAction={() => setCreating(true)} />}
      ...
      {loveNotes.map((note) => {
        ...
        const authorName = note.from === 'partner' ? (partner?.nickname || partner?.name || 'Pasangan') : (profile?.nickname || profile?.name || 'Kamu');
        return (
          <article key={note.id} style={cardStyle(note.state)}>
            <button
              type="button"
              disabled={!readable}
              title={readable ? undefined : 'Surat ini belum waktunya dibuka'}
            ...
            >
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10 }}>
                <div style={{ minWidth: 0 }}>
                  ...
                  <div style={pcss("font:600 10.5px 'Nunito',sans-serif;color:var(--mut,#A99A9E);margin-top:6px")}>{note.meta.replace('pasangan', authorName)}</div>
                </div>
              ...
              {readable && <div style={pcss("margin-top:12px;display:inline-block;padding:9px 15px;border-radius:100px;background:var(--pk,#FFB7B2);color:#5C3A42;font:700 11.5px 'Nunito',sans-serif")}>{expanded ? 'Tutup surat' : note.state === 'done' ? 'Baca lagi 💌' : 'Buka sekarang 💌'}</div>}
              {note.state === 'lock' && <div style={pcss("margin-top:10px;font:600 11px 'Nunito',sans-serif;color:var(--mut,#A99A9E)")}>Eits, sabar belum waktunya! 🤫</div>}
            </button>
